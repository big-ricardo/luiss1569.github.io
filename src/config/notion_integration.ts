...
const convertDataToObj = (item: any, key: string) => {
  switch (item.type) {
    case 'rich_text':
      ...
      return { [key]: item.rich_text.length > 1 ? item.rich_text.map(text => text?.plain_text) : item.rich_text[0]?.plain_text }
      break
    case 'title':
    ...
    case 'files':
      ...
      return { [key]: item.files.length > 1 ? item.files.map(arq => arq.file.url) : item.files[0].file.url }
      break
    case 'checkbox':
